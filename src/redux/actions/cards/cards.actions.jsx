...
export const sendNewVirtualCardInfo = (newVirtualCardInfo, token, hasOtp)=>{
    ...
    return (dispatch) =>{
        ...
        // smartech('dispatch', 'ALAT_VCard_Success', {
        //     "Email": user.email,
        //     "mobile": user.phoneNo
        // });
        return consume
...
export const deleteAlatVirtualCard = (payload, token)=>{
    ...
    return (dispatch)=>{
        ...
        let user = JSON.parse(user_details)
        // smartech('identify', user.email);
        // smartech('dispatch', 'ALAT_VCard_Delete', {
        //     "Email": user.email,
        //     "mobile": user.phoneNo
        // });
        return consume
...
export const getCurrentATMCard = (token)=>{
    ...
    return (dispatch)=>{
        ...
        console.log(user)
        // smartech('identify', user.email);
        // smartech('dispatch', 'ALAT_Request_Card', {
        //     "Email": user.email,
        //     "mobile": user.phoneNo
        // }); 
        return consume
...
export const getATMCardHotlistReasons = (token)=>{
    ...
    return (dispatch)=>{
        ...
        let user = JSON.parse(user_details)
        // smartech('identify', user.email);
        // smartech('dispatch', 'ALAT_Card_Hotlist_Success', {
        //     "Email": user.email,
        //     "mobile": user.phoneNo
        // });
...
export const hotlistATMCard = (payload, token)=>{
    ...
    return (dispatch)=>{
        ...
        let user = JSON.parse(user_details)
        // smartech('identify', user.email);
        // smartech('dispatch', 'ALAT_Card_Hotlist', {
        //     "Email": user.email,
        //     "mobile": user.phoneNo
        // });
        return consume
...
export const getALATCardSettings = (token, pan, isWemaCustomer)=>{
    SystemConstant.HEADER['alat-token'] = token;  
            ...
            return consume
                .then(response=>{
                    if(response.data.cardList.length>=1){
                        ...
                        let user = JSON.parse(user_details)
                        // smartech('identify', user.email);
                        // smartech('dispatch', 'ALAT_Card_Control', {
                        //     "Email": user.email,
                        //     "mobile": user.phoneNo
                        // });
                        return consume2
    ...
    function request(request) { return { type:GETALAT_CARDSETTINGS_PENDING, request, pan:panNum } }
    function success(response) { return {type:GETALAT_CARDSETTINGS_SUCCESS, response, pan:panNum} }
                                                    ...
                                                    dispatch(request(consume6));
                                                        return consume6
                                                            .then(response6=>{
                                                                ...
                                                                existingcardDetails = {
                                                                    // cardsList: response5.data.cardList,
                                                                    // cardsList:       response5.data.cardList,
                                                                    cardDesignId    : [],
                                                                    allCardDesigns  : response2.data,
                                                                    customerAccounts: response4.data,
...
export const postDataForNewATMCard = (payload, token)=>{
    ...
    return(dispatch)=>{
        ...
        let user = JSON.parse(user_details)
        // smartech('identify', user.email);
        // smartech('dispatch', 'ALAT_Request_Card_Success', {
        //     "Email": user.email,
        //     "mobile": user.phoneNo
        // }); 
        return consume
